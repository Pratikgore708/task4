...
# ----------------------------- Configuration ----------------------------- #
# Basic selectors for quotes.toscrape.com; change if you target another site.
QUOTE_SELECTOR = ("span", {"class": "text"})
AUTHOR_SELECTOR = ("small", {"class": "author"})
NEXT_PAGE_STRATEGIES = [
    # 1) <li class="next"><a href="...">Next</a></li>
    {"css": ("li", {"class": "next"}), "child": ("a", {})},
    # 2) <a rel="next" href="...">Next</a>
...
def extract_items_from_soup(soup: BeautifulSoup) -> List[QuoteItem]:
    """
    Extract quotes + authors from a parsed page using basic tag+class selectors.
    """
...
def find_next_page_url(soup: BeautifulSoup, base_url: str) -> Optional[str]:
    """
    Try common patterns to locate the 'next page' link.
...
def scrape_single_page(url: str) -> List[QuoteItem]:
    """Scrape one page and return QuoteItem list."""
    resp = http_get(url)
    if resp is None:
        return []
...
def scrape_with_pagination(start_url: str, max_pages: Optional[int] = None) -> List[QuoteItem]:
    """
    Scrape multiple pages following 'next' links.
...
def save_to_sqlite(items: List[QuoteItem], db_path: str = DEFAULT_DB, table: str = "quotes") -> None:
    """
    Save to SQLite with a UNIQUE constraint on (quote, author) to avoid duplicates.
    """
